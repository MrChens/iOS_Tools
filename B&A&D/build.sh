...
ApplicationLoader_UserName="zhengluff@chinanetcenter.com"
ApplicationLoader_Password="iTunes Producer: zhengluff@chinanetcenter.com"


BUGTRACKER_URL="svn://chenshilong@192.168.24.144:8888/tm/flow_browser/BugTracker"
...
function build_enterprise() {
    ...
    echo 'clear...'
    xcodebuild clean -project "$ARCHIVE_PROJECT_NAME" -configuration "$CONFIGURATION_TYPE" -quiet
    ...
    xcodebuild archive -project ./"$ARCHIVE_PROJECT_NAME" -scheme "$ARCHIVE_SCHEME_NAME" -configuration "$CONFIGURATION_TYPE" -archivePath "$ARCHIVE_FILE_PATH" PROVISIONING_PROFILE="Automatic"#CODE_SIGN_IDENTITY="iPhone Developer" -quiet > $ARCHIVE_LOG_FILE_NAME #PROVISIONING_PROFILE="Automatic" #-quiet > /dev/null #貌似不指定provisioning_profile才是真的automatic
    checkXcodebuildExitCode
    rm -r "build"
    echo 'exportArchive...'
    xcodebuild -exportArchive -archivePath "$ARCHIVE_FILE_PATH" -exportPath "$CUR_PATH" -exportOptionsPlist ./exportOptions/$3.plist
    ...
    rm -i -f "$OUTPUT_IPA_ENTERPRISE`$APP_VERSION`/"*.ipa

    cp -i "$ARCHIVE_SCHEME_NAME.ipa" "$OUTPUT_IPA_ENTERPRISE`$APP_VERSION`/${IPA_PREFIX_ENTERPRISE}${EXPORT_TIME}.ipa"
...
function usage() {
    ...
    echo -e "command:"
    echo -e " ./build.sh reset                  reset wscloudConfig.plist,WSPXUOneDefine.h to the publish mode."
    echo -e " ./build.sh archive.config         reset wscloudConfig.plist,WSPXUOneDefine.h and archive project then output ipa."
    echo -e ""
}
...
if [[ $1 == 'reset' ]]; then
        ...
        if [[ $TYPE == 'enterprise' ]]; then
            build_enterprise $VERSION $EP_BUILDVERSION enterprise Release
