...
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static NSString  *cslOrigAdd = nil;
static UInt16 cslremotePort;
#define wspxTestSocket 1
...
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		[self endConnectTimeout];
#if (wspxTestSocket)
      NSString * content = [NSString stringWithFormat:@"CONNECT %@:%d HTTP/1.1\r\n\r\n", cslOrigAdd, cslremotePort];
      NSData * data = [content dataUsingEncoding:NSUTF8StringEncoding];
      [self writeData:data withTimeout:5 tag:9999];
      NSLog(@"CSL WriteStreamRead");
      theFlags |= 1 << 15;
#else
...
{
#if (wspxTestSocket)
    UInt16 cutBuffer = 1<< 15;
    if ((theFlags & cutBuffer) && (theReadStream != NULL) && CFReadStreamHasBytesAvailable(theReadStream)) {
        theFlags &= ~cutBuffer;
        UInt8 buf[39] = {0};
//        CFReadStreamRead(theReadStream, buf, 39);
        NSLog(@"CSL readStreamRead");
        [self readIntoBuffer:buf maxLength:39];
        if ([theDelegate respondsToSelector:@selector(onSocket:didConnectToHost:port:)])
        {
            [theDelegate onSocket:self didConnectToHost:[self connectedHost] port:[self connectedPort]];
        }
