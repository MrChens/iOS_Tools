...
2. 从`Binary Images:
0x102ce4000 - 0x102ceffff wspxDemo arm64  <b0ffd72fc5c33a59bf97c79556430202> /var/containers/Bundle/Application/DF7AAB28-9FF0-99EC-4EA1-16B5C50EB87C/wspxDemo.app/wspxDemo`中我们拿到`b0ffd72fc5c33a59bf97c79556430202`
...
10. 从`crash`文件中我们很容易定位到`app`挂在了`AppDelegate.m`文件中的`第150行`这里.并且可能是因为数组越界导致崩溃。从源码截图中看确实是在`AppDelegate.m`的`150行`有问题.
11. 得到符号化`crash`文件后就可以进一步排查问题，之前有写过相关的博客[传送门][debug-with-crashlog]在这里就不再详细说明.
